...
function showResult(questionId) {
  if(answers[questionId] == useranswers[questionId]) {
    document.getElementById('result_' + questionId).innerHTML = '<img class="myImg" src="images/phase02/p2_mcq/blank.gif" style="border:0" alt="Correct!" />';
  } else {
    document.getElementById('result_' + questionId).innerHTML = '<img class="myImg1" src="images/phase02/p2_mcq/blank.gif" style="border:0" alt="Incorrect!" />';
  }
}
function showScore() {
  ...
  if(pc == 100){
	...
	for(var i=0; i<myImgC.length; i++){
		myImgC[i].src = "images/phase02/p2_mcq/correct.png";
	}
  }
  else if(pc >= 75)
    alertMsg += response[1];
	...
	for(var i=0; i<myImgIC.length; i++){
		myImgIC[i].src = "images/phase02/p2_mcq/incorrect.gif";
	}
  }
  else if(pc > 25)
    alertMsg += response[3];
