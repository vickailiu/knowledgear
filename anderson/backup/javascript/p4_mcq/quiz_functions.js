...
function showResult(questionId) {
  if(answers[questionId] == useranswers[questionId]) {
    document.getElementById('result_' + questionId).innerHTML = '<img class="myImg" src="images/phase04/p4_mcq/blank.gif" style="border:0" alt="Correct!" />';
  } else {
    document.getElementById('result_' + questionId).innerHTML = '<img class="myImg1" src="images/phase04/p4_mcq/blank.gif" style="border:0" alt="Incorrect!" />';
  }
}
function showScore() {
  ...
  if(pc == 100){
	...
	for(var i=0; i<myImgC.length; i++){
		myImgC[i].src = "images/phase04/p4_mcq/correct.png";
	}
	dataToJSON += ']}';
	...
	for(var i=0; i<myImgIC.length; i++){
		myImgIC[i].src = "images/phase04/p4_mcq/incorrect.gif";
	}
  }
  else if(pc > 25)
    alertMsg += response[3];
